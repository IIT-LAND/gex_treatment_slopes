...
#------------------------------------------------------------------------------
# FOR MP REVISION -------------------------------------------------------------
# add intensity column to pheno_data_sub
tmp_tx = tx_data
colnames(tmp_tx)[colnames(tmp_tx)=="STAR_avg_hoursPerWeek_Tx"] = "intensity"
pheno_data_sub = merge(pheno_data_sub, tmp_tx[,c("subjectId","intensity")], by="subjectId", sort = FALSE)
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# FOR MP REVISION -------------------------------------------------------------
# change column name for treatment intensity to "intensity"
colnames(tx_data_sub)[colnames(tx_data_sub)=="STAR_avg_hoursPerWeek_Tx"] = "intensity"
#------------------------------------------------------------------------------
...
# Examine if treatment slopes are different as a function of batch, gex age, RIN, sex, or treatment intensity (average hours per week) 

```{r, warning=FALSE, message=FALSE}
#-----------------------------------------------------------------------------
# FOR MP REVISION -------------------------------------------------------------
# mod2use = lm(formula = rfx_slope ~ batch + gex_age + RIN + sex, data = pheno_data_sub)
mod2use = lm(formula = rfx_slope ~ batch + gex_age + RIN + sex + intensity, data = pheno_data_sub)
anova(mod2use)
#-----------------------------------------------------------------------------
```
...
#------------------------------------------------------------------------------
# FOR MP REVISION -------------------------------------------------------------
# test tx start groups for difference in pre-treatment ELC
tres = t.test(formula = Mullen_ELC_Pre ~ TxStartSubgrp, data = tx_data_sub)
tres

# perm_tres = RVAideMemoire::perm.t.test(formula = Mullen_ELC_Pre ~ TxStartSubgrp, 
#                                        data = tx_data_sub, nperm = 10000)
# perm_tres

# test tx start groups for difference in treatment intensity
mod2use = lm(formula = intensity ~ TxStartSubgrp, data = tx_data_sub)
anova(mod2use)

# report descriptive stats for treatment intensity and split by early-late start subgroups
describeBy(tx_data_sub[,c("subjectId","intensity")])
describeBy(tx_data_sub[,c("subjectId","TxStartSubgrp","intensity","Mullen_ELC_Pre")], group = "TxStartSubgrp")
#------------------------------------------------------------------------------
```
...
#------------------------------------------------------------------------------
# FOR MP REVISION
# Add in to data2use the Mullen_ELC_Pre variable
# data2use = merge(melted_tx_data, tx_data_sub[,c("subjectId","days_in_tx")])
data2use = merge(melted_tx_data, tx_data_sub[,c("subjectId","days_in_tx","Mullen_ELC_Pre")])
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# FOR MP REVISION
# add treatment intensity and Mullen_ELC_Pre as a covariates
# fx_form = as.formula(sprintf("%s ~ days_in_tx + %s*%s", y_var, x_var, subgrp_var))
fx_form = as.formula(sprintf("%s ~ days_in_tx + intensity + Mullen_ELC_Pre + %s*%s", y_var, x_var, subgrp_var))
#------------------------------------------------------------------------------
rx_form = as.formula(sprintf("~ %s|subjectId",x_var))
...
#------------------------------------------------------------------------------
# explicit force variables to be factors
exprPhenoData$batch = factor(exprPhenoData$batch)
exprPhenoData$sex = factor(exprPhenoData$sex)
#------------------------------------------------------------------------------
...
# Run LASSO to predict subject-specific slopes with gene expression data for EARLY SUBJECTS ONLY

```{r, warning=FALSE, message=FALSE}
#------------------------------------------------------------------------------
# FOR MP REVISION
# run separate LASSO models for Early and Late Start subgroups
...
#------------------------------------------------------------------------------
early_subs = as.character(tx_data_sub$subjectId[tx_data_sub$TxStartSubgrp=="Before24Mo"])
subs2grab = as.character(pheno_data_sub$exprColName[is.element(pheno_data_sub$subjectId, early_subs)])
exprPhenoData_early = exprPhenoData[is.element(exprPhenoData$exprColName,subs2grab),]
for (i in 1:length(subs2grab)){
  subs2grab[i] = sprintf("X%s",subs2grab[i])
}
data2use_early = data2use[subs2grab,]

# late_subs = as.character(tx_data_sub$subjectId[tx_data_sub$TxStartSubgrp=="After24Mo"])
# subs2grab = as.character(pheno_data_sub$exprColName[is.element(pheno_data_sub$subjectId, late_subs)])
# exprPhenoData_late = exprPhenoData[is.element(exprPhenoData$exprColName,subs2grab),]
# for (i in 1:length(subs2grab)){
#   subs2grab[i] = sprintf("X%s",subs2grab[i])
# }
# data2use_late = data2use[subs2grab,]
#------------------------------------------------------------------------------


score.LOO.slope = .myLasso.LOO.new(inputData = t(data2use_early), 
                                   inputTrait = exprPhenoData_early, 
                                   dataType = dataType)
...
# Make the spaghetti plot 
tx_data2plot = merge(melted_tx_data, exprPhenoData_early[,c("subjectId","rfx_slope")], by="subjectId")
...
long_plot = long_plot + 
  theme(axis.text.x = element_text(size=fontSize),
        ...
        strip.text.x = element_text(size=fontSize),
        axis.title.y = element_text(size=fontSize))
ggsave(filename = here("plots","aSLPtrajectories_rfx_slope_colored_EARLYSUBS.pdf"), plot = long_plot)


df2plot = data.frame(predSlope = score.LOO.slope$data2plot$predVal, 
                     obsSlope = score.LOO.slope$data2plot$inputTrait, 
                     subjectId = exprPhenoData_early$subjectId, 
                     subgrp =exprPhenoData_early$subgrp)
...
p2 = ggplot(data = df2plot, aes(x = predSlope, y = obsSlope)) + 
  ...
  theme(axis.text.x = element_text(size=fontSize_tmp),
        ...
        strip.text.x = element_text(size=fontSize_tmp),
        axis.title.y = element_text(size=fontSize_tmp))
ggsave(filename = here("plots","gex_lasso_loo_plot2a_EARLYSUBS.pdf"))

p_final = single_sub_plot + p2 + plot_layout(nrow=1,ncol=2)
ggsave(filename = here("plots","gex_lasso_loo_plot2b_EARLYSUBS.pdf"))
p_final
#------------------------------------------------------------------------------

# now lets find the genes that most highly contribute to the model
score.LOO.slope.genesALL = .myLasso.LOO.genes.new(inputData = t(data2use_early),
                                                  inputTrait = exprPhenoData_early,
                                                  dataType = dataType)
...
res = data.frame(geneSymbol = good_genes, genesALL.keep.LOOslope)
colnames(res)[2] = "ILMN_GENE"
write.csv(res, file = here("results","res_lasso_loo_bestgenes_Microarray_EARLYSUBS.csv"))
background_list = metadata2use$geneSymbol[is.element(metadata2use$PROBE_ID,colnames(data2use_early))]
write(background_list, file = here("results","backgroundList_Microarray_EARLYSUBS.csv"))
res

# permutation test on MSE
null_mse = .myRandFn.LOO.new(inputData = t(data2use_early),
                             inputTrait = exprPhenoData_early,
                             nperm = nperm,
                             dataType = dataType,
                             seed2use=123,
...
# Run LASSO to predict subject-specific slopes with gene expression data for LATE SUBJECTS ONLY
...
score.LOO.slope = .myLasso.LOO.new(inputData = t(data2use_late), 
                                   inputTrait = exprPhenoData_late, 
                                   dataType = dataType)
...
# Make the spaghetti plot 
tx_data2plot = merge(melted_tx_data, exprPhenoData_late[,c("subjectId","rfx_slope")], by="subjectId")
...
long_plot = long_plot + 
  theme(axis.text.x = element_text(size=fontSize),
        ...
        strip.text.x = element_text(size=fontSize),
        axis.title.y = element_text(size=fontSize))
ggsave(filename = here("plots","aSLPtrajectories_rfx_slope_colored_LATESUBS.pdf"), plot = long_plot)


df2plot = data.frame(predSlope = score.LOO.slope$data2plot$predVal, 
                     obsSlope = score.LOO.slope$data2plot$inputTrait, 
                     subjectId = exprPhenoData_late$subjectId, 
                     subgrp =exprPhenoData_late$subgrp)
...
p2 = ggplot(data = df2plot, aes(x = predSlope, y = obsSlope)) + 
  ...
  theme(axis.text.x = element_text(size=fontSize_tmp),
        ...
        strip.text.x = element_text(size=fontSize_tmp),
        axis.title.y = element_text(size=fontSize_tmp))
ggsave(filename = here("plots","gex_lasso_loo_plot2a_LATESUBS.pdf"))

p_final = single_sub_plot + p2 + plot_layout(nrow=1,ncol=2)
ggsave(filename = here("plots","gex_lasso_loo_plot2b_LATESUBS.pdf"))
p_final
#------------------------------------------------------------------------------

# now lets find the genes that most highly contribute to the model
score.LOO.slope.genesALL = .myLasso.LOO.genes.new(inputData = t(data2use_late),
                                                  inputTrait = exprPhenoData_late,
                                                  dataType = dataType)
...
res = data.frame(geneSymbol = good_genes, genesALL.keep.LOOslope)
colnames(res)[2] = "ILMN_GENE"
write.csv(res, file = here("results","res_lasso_loo_bestgenes_Microarray_LATESUBS.csv"))
background_list = metadata2use$geneSymbol[is.element(metadata2use$PROBE_ID,colnames(data2use_late))]
write(background_list, file = here("results","backgroundList_Microarray_LATESUBS.csv"))
res

# permutation test on MSE
null_mse = .myRandFn.LOO.new(inputData = t(data2use_late),
                             inputTrait = exprPhenoData_late,
                             nperm = nperm,
                             dataType = dataType,
                             seed2use=123,
